...
if ($Action -eq 'SUN' -or ($Action -eq 'BOOT' -and $dynSun)) {
    if ($dynSun -and $regData.LastLat -and $regData.LastLon) {
        ...
        $times = [AutoDM.SunCalc]::GetSunTimes($lat, $lon, $now)
...
if ($Action -eq 'SYNC' -or $Action -eq 'BOOT' -or $Action -eq 'SUN') {
    $isLight = $false
    if ($dynSun -and $regData.LastLat -and $regData.LastLon) {
        $latStr = $regData.LastLat.ToString()
        $lonStr = $regData.LastLon.ToString()
        ...
        $times = [AutoDM.SunCalc]::GetSunTimes($lat, $lon, $now)
        if ($times[0] -ne [datetime]::MinValue) {
            $isLight = ($now.TimeOfDay -ge $times[0].TimeOfDay -and $now.TimeOfDay -lt $times[1].TimeOfDay)
        }
    } else {
